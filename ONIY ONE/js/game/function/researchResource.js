function researchTooltip(res_name){
	return getTooDoc('将你的研究类型改成'+colorText(res_name)[2]+'<hr><small>'+colorText(res_name)[2]+':<br>质量'+format(main['resource'][res_name]['PR']()[0])+'<br>研究价值'+format(main['resource'][res_name]['PR']()[1])+'<br>单位研究价值'+format(n(main['resource'][res_name]['PR']()[0]).mul(main['resource'][res_name]['PR']()[1]))+'<hr>注意:<br>研究时无法切换类型<br>取出研究会强制清除物质')
}
...
function ResearchBar(){
	...
	getNotNumDoc('ResearchResourceVoidText',format(player.voidBar)+`<br>上限:`+format(player.voidBarMax)+'<br>'+colorText('voidEnergy')[2]+txt2)
	...
	getNotNumDoc('ResearchResourcePointText',format(player.pointBar)+`<br>上限:`+format(player.pointBarMax)+'<br>'+colorText('researchPoint')[2]+txt3)
