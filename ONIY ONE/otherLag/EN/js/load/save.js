...
item:元素默认值.若存档中并不存在这个元素,那么用这个值代替.这个值会对大数字库的数字进行自动的处理.
*/
...
function hardReset(key){
    let realRest = player.realRestTimes
    let ac = [false,false]
    for(let i=0;i<=2;i++){
        ac[i] = player['achievement'+(i+1)]
    }
    realRest = player.realRestTimes.add(1)
    player = {
        realRestTimes:n(0),
        achievement1:ac[0],
        achievement2:ac[1],
    ...
    player.realRestTimes = n(realRest)
	save(key);
